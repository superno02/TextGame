...
| 單元測試 | ✅ 已完成 | 97 個 Swift Testing 測試案例（全通過），含 UI 測試共 101 個 |
...
### Engine 模組
`GameEngine`（`@Observable`）為遊戲邏輯核心，管理訊息列表、場景移動、攻擊、NPC 對話、商店交易、存檔，並在啟動時檢查所有模板載入狀態。

### Views 模組
負責所有使用者介面。GameView 為純 UI 層，將操作轉發給 GameEngine。子頁面（SkillView、InventoryView、StatusView、ShopView）接收 `PlayerCharacter` 參數顯示資料。ShopView 以 `.sheet` 呈現商店介面。
...
### Tests 模組
7 個測試檔涵蓋：角色初始化（20 項）、技能經驗與升級（9 項）、物品條件判斷（9 項）、模板載入驗證（15 項）、NPC 對話條件（6 項）、戰鬥公式與邏輯（26 項）、交易價格計算（11 項），共 97 個測試案例（不含 UI 測試）。
