...
- 使用 `.sheet()` 呈現彈窗選單（移動、攻擊、談話、商店）
- 子頁面（SkillView、InventoryView、StatusView、ShopView）接收 `PlayerCharacter` 參數，確保多存檔時顯示正確角色

### Engine Layer

- `GameEngine`（`@Observable`）為遊戲邏輯核心
- 在 GameView 的 `.task` 中以 `ModelContext` 注入方式初始化
- 管理：訊息列表、場景狀態、彈窗控制、戰鬥狀態、對話狀態、商店狀態
- 提供方法：moveToScene、attackMonster（完整戰鬥迴圈）、startDialogue / selectDialogueOption / endDialogue（樹狀對話）、buyItem、sellItem、saveGame
- 內含 `CombatCalculator`（戰鬥純函數）、`TradeCalculator`（交易純函數）與 `CombatMonster`（運行時怪物狀態）
...
- **Bundle JSON**：場景（`scenes.json`，11 個）、怪物（`monsters.json`，13 種）、NPC（`npcs.json`，10 個）、物品模板（`items.json`，30 種）、職業定義（`guilds.json`，5 種）、掉落表（`loot_tables.json`，11 張）
...
- NPC 對話條件格式：`"guild:05_02_warrior"`、`"circle:3"`、`"item:01_01_iron_sword"`、`"affinity:10"`（支援職業 / 等階 / 持有物品 / 好感度四種條件）
...
│   │   ├── ShopView.swift         # 商店頁面（購買/出售分頁）
│   │   └── StatusView.swift       # 屬性頁面（基本資訊、屬性、狀態、金幣）
│   ├── Models/                    # 資料模型
│   │   ├── Enums.swift            # 列舉定義
...
│   ├── NPCTemplateTests.swift     # NPC 樹狀對話、條件評估、商人動作測試
│   └── TradeTests.swift           # 交易價格計算測試
...
| Class | 責任 |
|-------|------|
| `GameEngine` | 遊戲邏輯核心，管理訊息、場景、戰鬥系統、對話、商店交易、存檔；持有 ModelContext 與所有 TemplateLoader 引用 |

### SwiftData Model

| Class | 責任 |
|-------|------|
| `PlayerCharacter` | 玩家角色持久化資料，初始屬性從 GuildTemplate 載入，含技能與背包關聯、金幣、NPC 好感度 |
...
| `NPCTemplateLoader.shared` | 載入 NPC JSON，支援商人篩選、樹狀對話查詢與條件過濾 |
...
| `DialogueNode` / `DialogueContext` | 樹狀對話節點（巢狀選項、條件、動作、goto 跳轉）與條件評估上下文 |
| `NPCShopItem` | NPC 商店物品定義 |
| `CombatMonster` | 戰鬥中怪物運行時狀態（可變 HP） |
| `CombatCalculator` | 戰鬥數值計算純函數（命中/閃避/傷害/逃跑公式） |
| `TradeCalculator` | 交易數值計算純函數（購買價/出售價公式） |
...
6. ~~測試覆蓋率為零~~ → 已新增 106 個 Swift Testing 測試案例
...
2. ~~**商店系統**~~ → 已實作完整物品交易系統（金幣、購買/出售、TradeCalculator 價格公式、NPC 庫存管理、交易技能加成）
